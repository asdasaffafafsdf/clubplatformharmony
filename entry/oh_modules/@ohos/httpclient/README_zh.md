...
OpenHarmony ohpm环境配置等更多内容，请参考[如何安装 OpenHarmony ohpm包](https://gitcode.com/openharmony-tpc/docs/blob/master/OpenHarmony_har_usage.md)
...
创建client、request对象，使用NetAuthenticator对象对用户名和密码加密进行身份authentication
...
https://gitcode.com/openharmony/docs/blob/master/zh-cn/application-dev/reference/apis/js-apis-http.md#responsecode
...
| skipRemoteValidation     | boolean                    | Request.Builder | 当前请求设置是否跳过证书验证,默认为false                                                           |
...
| authenticator     | aAuthenticator                 | HttpClient.Builder | 为HTTP请求客户端添加身份authentication，可以在请求头中添加账号密码等信息。       |
...
| authenticate | request: Request, response: Response | Request | 对请求头添加身份authentication凭证    |
...
|     |---- library  # httpclient 库文件夹
            ...
            |---- Address.ts  # 请求地址。 
            |---- authenticator  # 身份authentication模块，用于提供网络请求401之后身份authentication。 
...
使用过程中发现任何问题都可以提[Issue](https://gitcode.com/openharmony-tpc/httpclient/issues) 给组件，当然，也非常欢迎给组件提[PR](https://gitcode.com/openharmony-tpc/httpclient/pulls)。

## 开源协议

本项目基于 [Apache License 2.0](https://gitcode.com/openharmony-tpc/httpclient/blob/master/LICENSE)，请自由地享受和参与开源。
