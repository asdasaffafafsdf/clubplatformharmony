...
# Windows 控制台 UTF-8 支持，避免 emoji 等字符导致 GBK 编码错误
...
API_KEY = "sk-8e4b75603a5944eb88df903d9b7b969f"
...
def chat():
    try:
        ...
        print("API 状态码:", res.status_code)
        print("API 原始返回：", res.text[:500])

        if res.status_code != 200:
            return make_response(500, f"AI API 返回错误({res.status_code}): {res.text[:300]}", None), 500

        resJson = res.json()

        # 检查是否有错误信息
        if 'choices' not in resJson:
            error_msg = resJson.get('error', {}).get('message', str(resJson))
            print("AI API 错误:", error_msg)
            return make_response(500, f"AI API 错误: {error_msg}", None), 500
...
def member_check_search():
    ...
    try:
        ...
        if not name:
            return jsonify({"code": 1, "msg": "请输入搜索姓名"})
        ...
        return jsonify({"code": 0, "msg": "搜索成功", "data": members})
    except Exception as e:
        cur.close()
        conn.close()
        return jsonify({"code": 1, "msg": f"搜索失败：{str(e)}"})
...
def member_check_update(id):
    ...
    try:
        ...
        if not name and not student_id:
            return jsonify({"code": 1, "msg": "至少需要提供姓名或社团账号"})

        cur.execute(
            "UPDATE members SET name = %s, student_id = %s WHERE id = %s",
            (name, student_id, id)
        )
        conn.commit()
        ...
        if cur.rowcount == 0:
            cur.close()
            conn.close()
            return jsonify({"code": 1, "msg": "成员不存在"})
