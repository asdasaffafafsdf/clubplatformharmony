...
export class CallServerInterceptor implements Interceptor {
    ...
    async intercept(chain: Chain): Promise<Response> {
        ...
        if (!!!this.dns && userRequest.getCa().length !==0 && !userRequest.isDnsInterceptor) {
        ...
        let address: Address
        let hostOrIp: string
        let isIp: boolean = false
        let isIPv6: boolean = false
        
        try {
            httpUrl = HttpUrl.get(parsedUrl)
            address = new Address(httpUrl.host, httpUrl.port)
            hostOrIp = Utils.getDomainOrIp(parsedUrl)
            let regexp = new RegExp("(\\d*\\.){3}\\d*")
            isIPv6 = Utils.isIPv6(hostOrIp)
            isIp = regexp.test(hostOrIp)
        } catch (err) {
            Logger.error('CallServerInterceptor: Failed to parse URL: ' + parsedUrl + ', error: ' + JSON.stringify(err))
            throw err
        }
        ...
        if (!isIp && !isIPv6) {
             if (!!this.dns) {
                 // 重定向时需要检查域名是否改变，如果改变则重置 selection
                 let needNewSelection = false
                 if (!!this.routeSelector) {
                     // 检查地址是否改变（重定向到新域名）
                     let currentAddress = this.routeSelector.address
                     if (currentAddress.uriHost !== address.uriHost || currentAddress.uriPort !== address.uriPort) {
                         needNewSelection = true
                         this.selection = null // 重置 selection
                         Logger.info('CallServerInterceptor: Address changed, reset selection')
                     }
                 }
                 
                 // 每次都重新创建 RouteSelector（基于当前请求的 address）
                 this.routeSelector = new RouteSelector(this.dns, chain, proxy, address)
                 
                 if (!!!this.selection || needNewSelection) {
                     // 首次请求或重定向到新域名：创建新的 selection
                     ...
                     if (this.selection.hasNextRoute()) {
                         ...
                         // 当前 selection 没有更多 route，获取新的 selection
                         this.selection = await this.routeSelector.next()
                         route = this.selection.next()
                         if (route.proxy.type == Type.DIRECT) {
                             parsedUrl = parsedUrl.replace(hostOrIp, route.add.uriHost)
        ...
        if (!!this.dns || userRequest.isDnsInterceptor || userRequest.skipRemoteValidation || !!this.certificateManager || !!this.certificatePinner) {
            options['route'] = route
            let httpRequest = Http.createHttp();
            httpRequest.setSkipRemoteValidation(userRequest.skipRemoteValidation);
            if (!userRequest.skipRemoteValidation){
